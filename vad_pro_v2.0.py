...
def raw_wav_write (ret_str,data_dir_raw,mark):
    ...
    wav_path = data_dir_raw + str(datetime.datetime.now()) + mark + ".wav"
...
def VAD_PRO(path_wav):
    ...
    before_after = 800     # 8000*0.1
    max_sent = 128000       # 8000*16
...
if __name__ == '__main__':
    ...
    # 批量处理
    filedir = os.walk("./datasets/data_rec")
    for root, dirs, files in filedir:
        for file in files:
            path_wav = os.path.join(root , file)
            print (path_wav)
            sent_data, color = VAD_PRO(path_wav)
            print (len(sent_data))
            print (color)